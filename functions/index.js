...
exports.scheduledCalcMostPopular = functions.pubsub.schedule('every 1 hours').onRun((context) => {
    ...
    .then(snapshot => {
        ...
        Object.keys(db.clicks).forEach(pomId => {
        ...
        const topAll = top.slice(0,20);
        const oneMonthMillis = 1000*60*60*24*30;
        const topMonth = top.filter(o => Date.now() - db.pom[o.id].createTime < oneMonthMillis).slice(0,20);
